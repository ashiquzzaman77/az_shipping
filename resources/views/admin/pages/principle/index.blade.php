<x-admin-app-layout :title="'Principle All'">
    ...
    <div class="card card-flash">
        <div class="card-header mt-6">
            ...
            <div class="card-toolbar">
                ...
                <a href="{{ route('admin.principle.create') }}" class="btn btn-light-primary">
                    ...
                    @foreach ($items as $key => $item)
                        <tr>
                            ...
                            <td class="">
                                ...
                                <img class=""
                                    src="{{ !empty($item->thumbnail_image) ? url('storage/' . $item->thumbnail_image) : 'https://ui-avatars.com/api/?name=' . urlencode('SS') }}"
                            ...
                            <td>
                                @if (Auth::guard('admin')->user()->can('edit.principle'))
                                    <a href="{{ route('admin.principle.edit', $item->id) }}" class="text-primary">
                                        <i class="fa-solid fa-pencil text-primary"></i>
                                    </a>
                                @endif

                                @if (Auth::guard('admin')->user()->can('delete.principle'))
                                    <a href="{{ route('admin.principle.destroy', $item->id) }}" class="delete">
        ...
        <script>
            $(document).ready(function() {
                $('.status-toggle').change(function() {
                    ...
                    $.ajax({
                        url: '/admin/principle/status/' + bannerId,
                        method: 'PUT',
                        data: {
                            _token: '{{ csrf_token() }}',
                            status: newStatus
                        ...
                        success: function(response) {
                            if (newStatus === 'active') {
                                toastr.success('Principle has been activated successfully.');
                            } else {
                                toastr.error('Principle has been deactivated successfully.');
                            }
                        },
                        error: function(xhr) {
