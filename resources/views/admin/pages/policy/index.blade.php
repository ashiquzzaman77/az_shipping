<x-admin-app-layout :title="'Terms and Policy'">
    ...
    <div class="card card-flash">
        <div class="card-header mt-6">
            ...
            <div class="card-toolbar">
                ...
                <a href="{{ route('admin.policy.create') }}" class="btn btn-light-primary">
                ...
                <tbody class="fw-bold text-gray-600">
                    ...
                    @foreach ($items as $key => $item)
                        <tr>
                            ...
                            <td>
                                <h6>{{ $item->effective_date }}</h6>
                            </td>
                            <td>
                                <h6>{{ $item->expiration_date }}</h6>
                            </td>
                            <td>
                                <h6>{{ $item->version }}</h6>
                            </td>
        ...
        <script>
            $(document).ready(function() {
                $('.status-toggle').change(function() {
                    ...
                    $.ajax({
                        url: '/admin/policy/status/' + bannerId,
                        method: 'PUT',
                        data: {
                            _token: '{{ csrf_token() }}',
                            status: newStatus
                        ...
                        success: function(response) {
                            if (newStatus === 'active') {
                                toastr.success('Policy has been activated successfully.');
                            } else {
                                toastr.error('Policy has been deactivated successfully.');
                            }
                        },
                        error: function(xhr) {
