<x-admin-app-layout :title="'Client All'">
    ...
    <div class="card card-flash">
        <div class="card-header mt-6">
            ...
            <div class="card-toolbar">
                ...
                <a href="{{ route('admin.client.create') }}" class="btn btn-light-primary">
                    ...
                    @foreach ($items as $key => $item)
                        <tr>
                            ...
                            <td class="">
                                ...
                                <img class=""
                                    src="{{ !empty($item->image) ? url('storage/' . $item->image) : 'https://ui-avatars.com/api/?name=' . urlencode("C") }}"
                            ...
                            <td>
                                ...
                                @if (Auth::guard('admin')->user()->can('edit.client'))
                                    <a href="{{ route('admin.client.edit', $item->id) }}" class="text-primary">
                                        <i class="fa-solid fa-pencil text-primary"></i>
                                    </a>
                                @endif

                                @if (Auth::guard('admin')->user()->can('delete.client'))
                                    <a href="{{ route('admin.client.destroy', $item->id) }}" class="delete">
        ...
        <script>
            $(document).ready(function() {
                $('.status-toggle').change(function() {
                    ...
                    $.ajax({
                        url: '/admin/client/status/' + bannerId,
                        method: 'PUT',
                        data: {
                            _token: '{{ csrf_token() }}',
                            status: newStatus
                        ...
                        success: function(response) {
                            if (newStatus === 'active') {
                                toastr.success('Client has been activated successfully.');
                            } else {
                                toastr.error('Client has been deactivated successfully.');
                            }
                        },
                        error: function(xhr) {
