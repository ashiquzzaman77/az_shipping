<x-admin-app-layout :title="'Team All'">
    ...
    <div class="card card-flash">
        <div class="card-header mt-6">
            ...
            <div class="card-toolbar">
                ...
                <a href="{{ route('admin.team.create') }}" class="btn btn-light-primary">
                    ...
                    @foreach ($items as $key => $item)
                        <tr>
                            ...
                            <td class="">
                                ...
                                <img class=""
                                    src="{{ !empty($item->image) ? url('storage/team/' . $item->image) : 'https://ui-avatars.com/api/?name=' . urlencode($item->name) }}"
        ...
        <script>
            $(document).ready(function() {
                $('.status-toggle').change(function() {
                    ...
                    $.ajax({
                        url: '/admin/team/status/' + bannerId,
                        method: 'PUT',
                        data: {
                            _token: '{{ csrf_token() }}',
                            status: newStatus
                        ...
                        success: function(response) {
                            if (newStatus === 'active') {
                                toastr.success('Team has been activated successfully.');
                            } else {
                                toastr.error('Team has been deactivated successfully.');
                            }
                        },
                        error: function(xhr) {
