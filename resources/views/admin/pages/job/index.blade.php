<x-admin-app-layout :title="'Job All'">
    ...
    <div class="card card-flash">
        <div class="card-header mt-6">
            ...
            <div class="card-toolbar">
                ...
                <a href="{{ route('admin.job.create') }}" class="btn btn-light-primary">
            ...
            <table id="kt_datatable_example_5" class="table table-striped table-row-bordered gy-5 gs-7 border rounded">
                <thead class="bg-dark text-light">
                    <tr>
                        ...
                        <th width="10%">Rank</th>
                        <th width="10%">Contact Duration</th>
                        <th width="10%">Experienced</th>
                        <th width="10%">Salary</th>
                        <th width="10%">Deadline</th>
                        <th width="10%">Total Needed</th>
                ...
                <tbody class="fw-bold text-gray-600">
                    ...
                    @foreach ($items as $key => $item)
                        <tr>
                            ...
                            <td>
                                <h6>{{ $item->contract_duration }}</h6>
                            </td>
                            <td>
                                <h6>{{ $item->experienced }}</h6>
                            </td>
                            <td>
                                <h6>{{ $item->salary }}</h6>
                            </td>
                            <td>
                                <h6>{{ \Carbon\Carbon::parse($item->deadline)->format('d M Y') }}</h6>
                            </td>
                            <td>
                                <h6>{{ $item->total_needed }}</h6>
                            </td>
                ...
                $('.status-toggle').change(function() {
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            if (newStatus === 'active') {
                                toastr.success('Job has been activated successfully.');
                            } else {
                                toastr.error('Job has been deactivated successfully.');
                            }
                        },
                        error: function(xhr) {
